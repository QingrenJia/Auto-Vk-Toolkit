...
namespace avk
{
	...
	/** Divides the given meshlets into groups.
	 *  @param	tVertices			The vertex buffer.
	 *  @param	aIndices			The index buffer.
	 *  @param	aModel				The model these buffers belong to.
	 *	@param	aMeshIndex			The optional mesh index of the mesh these buffers belong to.
	 *  @param	recived_meshlets	The provided meshlets for aModel.
	 *	@param	aMaxVertices		The maximum number of vertices of a meshlet.
	 *	@param	aMaxIndices			The maximum number of indices of a meshlet.
	 *	@param	aMaxMeshlets		The maximum number of meshlets of a group.
	 */
	 ...
	 */
	template <typename F>
	std::vector<meshlet> divide_meshlets_into_groups(
		const std::vector<glm::vec3>& aVertices,
		const std::vector<uint32_t>& aIndices,
		avk::model aModel, 
		std::vector<meshlet>& recived_meshlets,
		const std::optional<mesh_index_t> aMeshIndex,
		const uint32_t aMaxVertices, const uint32_t aMaxIndices, const uint32_t aMaxMeshlets,
		F aGroupDivision)
	{
		std::vector<meshlet> generatedMeshlets;
		aModel.enable_shared_ownership();

		if constexpr (std::is_assignable_v<std::function<std::vector<meshlet>(const std::vector<uint32_t>&tIndices, const model_t & tModel, std::optional<mesh_index_t> tMeshIndex, uint32_t tMaxVertices, uint32_t tMaxIndices)>, decltype(aGroupDivision)>) {
			generatedMeshlets = aGroupDivision(aIndices, aModel.get(), aMeshIndex, recived_meshlets, aMaxVertices, aMaxIndices, aMaxMeshlets);
		}
		else if constexpr (std::is_assignable_v<std::function<std::vector<meshlet>(const std::vector<glm::vec3>&tVertices, const std::vector<uint32_t>&tIndices, const model_t & tModel, std::optional<mesh_index_t> tMeshIndex, uint32_t tMaxVertices, uint32_t tMaxIndices)>, decltype(aGroupDivision)>) {
			generatedMeshlets = aGroupDivision(aVertices, aIndices, aModel.get(), aMeshIndex, recived_meshlets, aMaxVertices, aMaxIndices, aMaxMeshlets);
		}
		else {
			assert(false);
			throw avk::logic_error("No compatible lambda has been passed to divide_into_groups.");
		}
	...
	/** Divides the given meshlets into groups with each of group contains 4 meshlets using the given callback function.
	 *  @param	aModelsAndMeshletIndices				All the models and associated meshes that should be divided into meshlets.
	 *  @param	aGroupDivision	Callback used to divide meshlets into groups contains 4 meshlets with a maximum number of vertices and indices each.
	 *								It can either receive the vertices and indices or just the indices depending on your specific needs.
	 *								Additionally it provides the model and an optional mesh index if more data is needed. If no mesh index is provided then the meshes were combined beforehand.
	 *								Ownership of the model must not be taken within the body of aMeshletDivision. The model will be assigned to each `meshlet` after aMeshletDivision has executed. \n\n
	 *								The callback must follow a specific declaration schema, optional parameters can be omitted, but all of them need to be provided in the following order:
	 ...
	 */
	template <typename F>
	std::vector<meshlet> divide_into_groups(std::vector<std::tuple<avk::model, std::vector<avk::mesh_index_t>>>& aModelsAndMeshletIndices,
		std::vector<meshlet>& recived_meshlets, F aGroupDivision,
		const bool aCombineSubmeshes = true, const uint32_t aMaxVertices = 64, const uint32_t aMaxIndices = 378, const uint32_t aMaxMeshlets = 4)
	{
		std::vector<meshlet> meshlets;
		for (auto& pair : aModelsAndMeshletIndices) {
			auto& model = std::get<avk::model>(pair);
			...
			if (aCombineSubmeshes) {
				auto [vertices, indices] = get_vertices_and_indices(make_model_references_and_mesh_indices_selection(model, meshIndices));
				std::vector<meshlet> tmpMeshlets = divide_meshlets_into_groups(vertices, indices, std::move(model), recived_meshlets, std::nullopt, aMaxVertices, aMaxIndices, aMaxMeshlets, std::move(aGroupDivision));
				// append to meshlets
				meshlets.insert(std::end(meshlets), std::make_move_iterator(std::begin(tmpMeshlets)), std::make_move_iterator(std::end(tmpMeshlets)));
			}
			else {
				for (const auto meshIndex : meshIndices) {
					auto vertices = model.get().positions_for_mesh(meshIndex);
					auto indices = model.get().indices_for_mesh<uint32_t>(meshIndex);
					std::vector<meshlet> tmpMeshlets = divide_meshlets_into_groups(vertices, indices, std::move(model), recived_meshlets, meshIndex, aMaxVertices, aMaxIndices, aMaxMeshlets, std::move(aGroupDivision));
					// append to meshlets
					meshlets.insert(std::end(meshlets), std::make_move_iterator(std::begin(tmpMeshlets)), std::make_move_iterator(std::end(tmpMeshlets)));
				}
			} 
